...
import argparse
import os
import cal_ws_metrics  


def cal_ws_ssim_psnr(src_gt, src_sr,output_file, metrics=None, print_avg=True, print_details=True):
    '''
    ...
    with open(output_file, 'w') as f:
        ...
        for i, (pth_gt, pth_sr) in enumerate(zip(pths_gt, pths_sr)):
            ...
            if print_details:
                print_to_file(f'[{i}/{n}][idx]:{s_psnr}{s_ssim}')

        if print_avg:
            if 'wspsnr' in metrics:
                print_to_file(f'Average value of WS-PSNR:\t {output_dict["wspsnr_avg"]}')
            if 'wsssim' in metrics:
                print_to_file(f'Average value of WS-SSIM:\t {output_dict["wsssim_avg"]}')
...
def main():
    parser = argparse.ArgumentParser(description='Calculate WS-PSNR and WS-SSIM metrics')
    parser.add_argument('src_gt', type=str, help='Path to the ground truth images')
    parser.add_argument('src_sr', type=str, help='Path to the super-resolved images')
    parser.add_argument('--output_file', type=str, default='output_metrics.txt', 
                        help='Path to the output text file')
    # parser.add_argument('--metrics', type=str, nargs='*', default=['wspsnr', 'wsssim'],
    #                     help='List of metrics to compute (e.g., wspsnr, wsssim)')
    # parser.add_argument('--print_avg', action='store_true', default=True, 
    #                     help='Whether to print the average value of metrics')
    # parser.add_argument('--print_details', action='store_true', default=True, 
    #                     help='Whether to print detailed values for each image')

    args = parser.parse_args()
    if not os.path.isdir(args.src_gt):
        print(f"Error: Ground truth directory '{args.src_gt}' does not exist.")
        return
    if not os.path.isdir(args.src_sr):
        print(f"Error: Super-resolved directory '{args.src_sr}' does not exist.")
        return

    output = cal_ws_ssim_psnr(
        src_gt=args.src_gt,
        src_sr=args.src_sr,
...
# python calculate_metrics.py /path/to/gt_images /path/to/sr_images  --output_file swinir_sun_x4.txt
